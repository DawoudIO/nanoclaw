...
export function archiveClaudeTranscript(input: ClaudeArchiveInput, fx: ClaudeHistoryClock): boolean {
  ...
  try {
    ...
    pruneConversationsDir(conversationsDir, input.log);
    return true;
  } catch (error) {
 ...
 * write above is a full re-serialization of the whole conversation, not
 * incremental, and this directory otherwise has no cleanup of its own at
 * all — unlike the .jsonl resume-transcript it's read from, which already has
 * real rotation (rotateClaudeContinuation, above). A real install saw 264
 * files / 188MB in one day from this on one container, a strong candidate
 * cause of an OOM crash loop it hit that day. Same override pattern as the
 * transcript rotation knobs: 0/non-positive disables.
 */
 ...
 * fail the archive write that just succeeded above it. Called from every
 * archiveClaudeTranscript caller (PreCompact and continuation rotation
 * alike), so no archive-writing path can grow this directory unbounded.
 */
function pruneConversationsDir(conversationsDir: string, log: (message: string) => void): void {
